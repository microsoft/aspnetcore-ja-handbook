...
> [!WARNING]
> プールされた `DbContext` インスタンスは再利用されるため、実質的に Singleton のように扱われます。`OnConfiguring` はインスタンスの初期化時の 1 回しか呼ばれず、リクエストごとに変化する状態（テナント ID や現在のユーザーなど）の設定には使えません。**EF Core の内部状態のリセットで、アプリケーション独自のフィールドやプロパティまで初期化されるとは考えないでください。**
>
> リクエスト固有の状態が必要なら、プールしない `AddDbContext` を使うか、[公式の状態管理の例](https://learn.microsoft.com/ja-jp/ef/core/performance/advanced-performance-topics#managing-state-in-pooled-contexts)に従ってください。後者では、`AddPooledDbContextFactory` で登録したファクトリをスコープ付きのファクトリで包み、**プールから取得するたびに、そのリクエストの信頼できるテナント ID などを必ず上書きしてから渡します。** コンテキストの取得経路をこのファクトリに統一し、返却時のリセットフックに依存しない構成にします。テナント ID は要求パラメーターをそのまま信用せず、認証情報などから確定してください。
