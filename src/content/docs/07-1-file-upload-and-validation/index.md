...
> [!TIP]
> `OpenReadStream()` が呼び出しごとに独立したストリームを返すという性質は、地味ですが重要です。本章では後ほど「まず内容を検証し、そのあと保存する」という処理を書きますが、検証で先頭の数バイトを読んでも、保存時にもう一度 `OpenReadStream()` を呼べば先頭から読み直せます。読み取り位置を戻す処理は要りません。
>
> 逆に、1 つのストリームを取得して検証と保存で使い回すと、**検証で読んだ分だけ欠けたファイルが保存されます**。34 バイトのファイルで先頭 3 バイトを検証に使うと、保存されるのは 31 バイトです。エラーにはならず、壊れたファイルが静かに出来上がるため、必ず用途ごとに `OpenReadStream()` を呼んでください。
...
ストリーミング受信のコードは、`IFormFile` に比べると確かに長くなります。ただしこれは「ASP.NET Core がストリーミングをサポートしていない」ということではありません。`MultipartReader` はフレームワークが標準で提供する公式のユーティリティであり、公式ドキュメント「[ASP.NET Core でファイルをアップロードする](https://learn.microsoft.com/ja-jp/aspnet/core/mvc/models/file-uploads?view=aspnetcore-10.0)」でもストリーミングの推奨手段として案内されています。
...
なお、この 3,000 件には `E3` と `E8` は 1 件も現れませんでした。

> [!IMPORTANT]
> MS Learn のサンプルコード (`FileHelpers.cs`) は、**同じ JPEG でも拡張子の綴りによって異なる 3 パターンを許可** します。`.jpeg` は `E0` / `E2` / `E3`、`.jpg` は `E0` / `E1` / `E8` です。上の 3,000 件を当てはめると、拒否される件数は次のようになります。
>
> | 許可リスト | 拒否される件数 | 内訳 |
> | --- | --- | --- |
> | `.jpeg` に適用される `E0` / `E2` / `E3` | **363 件（約 12%）** | `E1` 352 件、`EE` 9 件、`DB` 2 件 |
> | `.jpg` に適用される `E0` / `E1` / `E8` | **153 件（約 5%）** | `E2` 142 件、`EE` 9 件、`DB` 2 件 |
>
> とりわけ `.jpeg` では、デジタルカメラやスマートフォンで撮影した写真の標準形式である **Exif JPEG が丸ごと拒否されます**。写真投稿機能でこの実装を使えば、スマートフォンから送られたファイルがことごとく失敗することになります。同じ画像なのに拡張子の綴りしだいで結果が変わる点も、利用者には理解しようのない挙動です。JPEG は `FF D8 FF` の 3 バイトで判定してください。
...
> [!NOTE]
> `AddValidation()` は **ソースジェネレーター**（コンパイル時にソースコードを解析し、追加のコードを自動生成する C# のしくみ。Java のアノテーションプロセッサーに相当します）を使って、**呼び出したアセンブリ内** の検証対象の型を検出します。そのため、Minimal API のエンドポイントを別のアセンブリ（クラスライブラリなど）で定義していると、ホスト側で `AddValidation()` を呼ぶだけでは効かない場合があります。実際に試すと、次のように分かれます。
>
...
> [!WARNING]
> `MaxFileSizeAttribute` の `IsValid` は、`value is not IFormFile file || ...` と書かれているため、**`value` が `null` のときは `true`（検証成功）を返します**。ファイルが送られてこなかった場合も `IsValid` 自体は `value` が `null` の状態で呼ばれますが、その結果が「妥当」と判定されてしまうわけです。これは `value is not T` という書き方をするカスタム属性に共通の落とし穴です。そのため `[MaxFileSize]` だけを付けたプロパティは「ファイルが送られてきた場合にサイズを検査する」という意味にしかならず、**ファイルを一切送らないリクエストは検証を素通りします**（実際に試すと HTTP 200 が返ります）。ファイルを必須にしたい場合は、②の例のように `[Required]` を併せて指定してください。なお①のように `IFormFile` を単体の引数として受け取る場合は、ファイルが送られてこないとバインド自体が失敗するため、`[Required]` がなくても HTTP 400 が返ります。
