...
保存先は、アプリケーションの配置ディレクトリの外にある専用領域とし、可能であれば実行権限を外してください。**後編で推奨する [Azure Blob Storage への保存](../07-2-azure-blob-storage/index.md)は、そもそもアプリケーションのファイルシステムと切り離されているため、この問題を構造的に回避できます**。
