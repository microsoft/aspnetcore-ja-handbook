---
title: "第2章：ソリューションとプロジェクト構成"
description: "ソリューションとプロジェクトの違いと役割、プロジェクト参照と NuGet パッケージ参照の使い分け、複数プロジェクト構造のパターン、ビルド／実行フロー、デバッグ設定・起動構成の管理を解説します。"
---
...
- NuGet パッケージ参照（ `<PackageReference>` ）および他プロジェクトへの参照（ `<ProjectReference>` ）— **（後述： [2. プロジェクト参照とNuGetパッケージ参照の使い分け](#2-プロジェクト参照とnugetパッケージ参照の使い分け) ）**
