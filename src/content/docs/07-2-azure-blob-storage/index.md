...
以降のコード例は、いずれもコントローラーのアクション部分だけを抜き出したものです。`_fileStorage` は前節で定義した `IFileStorage`、`_authorizationService` は ASP.NET Core 標準の `IAuthorizationService` を、それぞれコンストラクターで受け取っている前提とします。コントローラー全体の姿は[コントローラーからの利用](#コントローラーからの利用)で示します。
