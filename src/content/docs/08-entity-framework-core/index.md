...
> [!WARNING]
> 上の `ThenInclude` を重ねた例のように、**1 つのクエリで複数のコレクションを `Include` すると、結合の結果として親側の列が子の行数だけ繰り返され、転送量が急増します**（デカルト爆発）。EF Core はこれを避けるためにクエリを分割する `AsSplitQuery()` を用意しています。どちらを選ぶべきかと、分割した場合に生じる整合性の問題は[付録3の「単一クエリと分割クエリ」](../appendix-efcore-03/index.md#単一クエリと分割クエリ)で扱います。
