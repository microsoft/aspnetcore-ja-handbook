...
description: "MVC 構成の基本、コントローラー／アクションの書き方、ルーティング、モデルバインディングと入力検証、ControllerBase による API 実装、フィルターによる横断処理を解説します。"
---
...
4. **Multipart/form-data の推論** – Content-Type が multipart/form-data の場合の取り扱いも自動化され、ファイルアップロードシナリオで便利になります。例えばアクションに `IFormFile` パラメータがあれば、 `[FromForm]` と推定されます。受信したファイルの扱いと検証については[第7章（前編）：ファイル受信と検証](../07-1-file-upload-and-validation/index.md)で詳しく解説します。
...
- **リソースフィルター (Resource Filter)**: 認可フィルターの後、モデルバインドが行われる前に実行されます。ここではアクションメソッドの引数がセットされるより前の段階で介入できるため、キャッシュの短絡（キャッシュがあればアクションを実行しない）などに使われます。リソースフィルターはあまり頻繁には使われませんが、例えば大きなファイルのアップロード前に要求をチェックする、といった用途があります（アップロードサイズの上限そのものを設定で制御する方法は[第7章（前編）：既定のサイズ制限と設定変更](../07-1-file-upload-and-validation/index.md#既定のサイズ制限と設定変更)を参照）。カスタム実装は `IResourceFilter` または `IAsyncResourceFilter` を実装します。
...
また、認証・認可では `[Authorize]` 属性などを活用し、ロールベースやポリシーベースのアクセス制御を宣言的に付与できます（[第9章：認証と認可](../09-authentication-and-authorization/index.mdx)で解説します）。
