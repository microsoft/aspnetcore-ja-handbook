---
title: "第6章：依存性注入 (DI)"
description: "DI の基本概念とコンストラクターインジェクション、Transient／Scoped／Singleton のライフタイム、ファクトリ登録・キー付きサービス・手動スコープ、サービス層の設計パターンを解説します。"
---
...
| 生成経路 | 主な対象 | 複数コンストラクター時の挙動 |
| --- | --- | --- |
| `IServiceProvider` | DI コンテナーに登録した通常のサービス | DI で解決可能なパラメーターが最も多いコンストラクターが選択される。優劣を判定できない組み合わせがある場合は例外 |
| `ActivatorUtilities` | コントローラーなど、フレームワークが生成するクラス | **適用可能なコンストラクターが 1 つだけ** である必要がある。複数該当すると例外 |

> [!WARNING]
> コントローラーは `ActivatorUtilities` によって生成されます。そのため「解決可能なパラメーターが最も多いものが選ばれる」という規則は適用されず、DI で解決できるコンストラクターを複数定義すると次の例外が発生します。
>
> ```text
> System.InvalidOperationException: Multiple constructors accepting all given argument types
> have been found in type 'OrderController'. There should only be one applicable constructor.
> ```
>
> コントローラーのコンストラクターは 1 つだけ定義してください。
...
> [!WARNING]
> MVC コントローラーのアクションメソッドでは `[FromServices]` を省略できません。省略するとサービスではなくモデルバインディングの対象として扱われ、実行時に次のエラーになります。
>
> ```text
> System.InvalidOperationException: Could not create an instance of type 'IReportGenerator'.
> Model bound complex types must not be abstract or value types and must have a parameterless constructor.
> ```
>
> 一方、[第4章：Minimal API](../04-minimal-api/index.md) で扱う Minimal API のハンドラーでは、DI コンテナーに登録済みの型であれば属性を省略しても解決されます。
...
| --- | --- | --- | --- |
| **Transient** | `AddTransient<TService, TImpl>()` | サービスが要求されるたびに毎回 | 解決されたスコープの終了時（Web アプリでは通常リクエスト終了時） |
| **Scoped** | `AddScoped<TService, TImpl>()` | 同一スコープ（HTTP リクエスト）内で最初の要求時 | スコープの終了時（Web アプリでは通常リクエスト終了時） |
...
> [!WARNING]
> `IDisposable` を実装した Transient サービスを **ルートのサービスプロバイダー**（スコープを作らずアプリケーション全体のプロバイダー）から解決すると、コンテナーがインスタンスへの参照を保持し続け、アプリケーション終了時まで破棄されません。大量に解決するとメモリリークの原因になります。必ずスコープ内で解決してください。
...
> [!WARNING]
> キー付きで登録したサービスは、**キーを指定しない解決では取得できません**。上記の登録だけを行った状態で `GetService<ICache>()` を呼び出すと `null` が返ります。既存のキーなし登録をキー付き登録に変更すると、キーを指定していない既存の注入箇所がすべて解決できなくなる点に注意してください。両方の解決方法を使いたい場合は、キーなし登録も併せて行います。
...
| HTTP クライアント | Transient（`IHttpClientFactory` 経由） | 外部 API クライアント |
| ロギング | Singleton | `ILogger<T>`（フレームワーク登録済み） |
| バリデーション | Transient | バリデータ、フォーマッタ |

> [!WARNING]
> **ライフタイムの依存方向に注意してください。** 長いライフタイムのサービスから短いライフタイムのサービスへの依存は、短いライフタイムのサービスが長いライフタイム側に閉じ込められる **Captive Dependency** を引き起こします。
>
