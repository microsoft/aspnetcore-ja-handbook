...
記事は `src/content/docs/` 配下にあります。**1 つの章が 1 つのフォルダ** に対応し、本文は原則 `index.md` という名前になります。第9章はタブ表示を試すため `index.mdx` を使用しています。ほかの章を一括で MDX に変更する必要はありません。
...
| 決まりごと | 内容 |
|---|---|
| 本文のファイル名 | 原則 `index.md`。コンポーネントを使う章は `index.mdx`。同じフォルダに両方を置かないでください。どちらもフォルダ名が URL になります（`01-setup-dev-env/` → `/aspnetcore-ja-handbook/01-setup-dev-env/`） |
...
他の章へリンクするときは、**移動先の `index.md` または `index.mdx` までを含む相対パス** で書いてください。
...
`.md` / `.mdx` は公開サイトの URL へ自動的に変換されるため、そのまま書いて構いません。この書き方であれば、GitHub 上でも公開サイトでもリンク先のファイル・ページへ移動できます。ただし、MDX のコンポーネントは GitHub 上で公開サイトと同じようには表示されません。

第9章のタブは Starlight の `Tabs` / `TabItem` で本文を囲んでいます。`TabAnchorNavigation` は、目次や直接リンクから非表示タブ内の見出しへ移動した際に、そのタブを開く補助です。通常の Markdown の折りたたみに戻す場合は、これらの読み込みと補助コンポーネントの配置を削除し、タブの囲みを `<details>` / `<summary>` に置き換えてから、拡張子と参照リンクを `.md` に戻します。本文・コード・画像は通常の Markdown として記述してください。
