...
貢献の手段は GitHub 上での Issue を作成、または Discussion を立ててから Issue を作成、その上で Pull Request を作成する形になります。以下のセクションで詳しく説明します。
...
8. GitHub Copilot code review のレビューに対応する
   - Pull Request を作成すると、自動的に GitHub Copilot code review が起動し、あなたの Pull Request の内容をレビューします。必ずレビューの通りに修正しなければならない訳ではありませんが、従っておいた方が良い結果となることが多いはずです。修正したら、必ずレビューを「解決」にしてください。もし、レビューの内容がおかしかった場合は、ただ「解決」にしてください。
9. Pull Request が承認されて main に マージ されます
   - GitHub Copilot code review による全てのレビューが解決済みであり、CODEOWNER による手動レビューの後に main にマージされます。
