...
![Visual Studio 画面](./images/05-02_vs-secret-management.png)
...
`dotnet user-secrets init` を実行すると、プロジェクトファイル（`.csproj`）に `UserSecretsId` が追加されます。（Visual Studio を使用する場合は自動的に追加されるため、この操作は不要）
