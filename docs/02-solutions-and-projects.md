...
一方、 **ソリューション（ `.sln` ファイル）** は複数のプロジェクトを **まとめて管理するための最上位コンテナー** です。  
...
| `Program.cs` | アプリケーションのエントリーポイント。DI コンテナーへのサービス登録やミドルウェアパイプラインの構成を行う。 |
| `appsettings.json` / `appsettings.Development.json` | アプリケーション設定ファイル（接続文字列やログレベルなど）。環境名に対応したファイルが実行時にマージされる。 |
| `Properties/launchSettings.json` | ローカル実行時の起動プロファイル定義。使用する Web サーバ、ポート、環境変数などをプロファイルごとに設定する。 |
| `Controllers/` | MVC コントローラクラスを配置する。HTTP リクエストの受け取りとレスポンスの返却を担う。 |
| `Models/` | ビューモデルやデータモデルクラスを配置する。 |
| `Views/` | Razor ビュー（`.cshtml`）を配置する。コントローラと対応したサブフォルダ構成になっている。 |
...
ASP.NET Core のプロジェクトテンプレートも基本的に 1 つのプロジェクトの中でモデル・ビュー・コントローラやデータアクセス用のフォルダを分けておく形で始まります。  
...
- **Web プロジェクト** – プレゼンテーションとアプリケーションのエントリーポイント。ASP.NET Core MVC や API のコントローラ、UI 用のページなどを含み、ApplicationCore や Infrastructure からビジネス機能を呼び出す。プロジェクト形式は ASP.NET Core Web アプリとして構成。
...
対して ASP.NET Core Web アプリケーションや API といった Web アプリケーション形式の場合、実行すると組み込みの **Kestrel Web サーバ** が起動し、所定の URL（ポート）でリッスンを開始します。  
...
この `launchSettings.json` は **ローカル実行時のオプション設定（使用する Web サーバ、デバッグ有無、環境変数、URL など）を複数定義** でき、 **起動プロファイル** と呼びます。
...
`IIS Express` プロファイルは Windows 上の開発用 Web サーバ (IIS Express) でアプリを起動します。  
一方、`https` プロファイルでは Kestrel という組み込み Web サーバでアプリを起動し、 `applicationUrl` に指定したポートでリッスンします。（今後は、クロスプラットフォーム環境で一貫した動作を得られる Kestrel サーバの使用が推奨されています。VS Code や `dotnet run` を使う場合は Kestrel ( `commandName: "Project"` のプロファイル)を選択します。）
